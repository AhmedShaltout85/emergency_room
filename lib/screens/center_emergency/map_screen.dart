...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _checkConnectivity() async {
    ...
    // Note: no blocking dialog here on purpose. The station markers and
    // info card are built from a local, hardcoded map — they're always
    // on screen regardless of connectivity, so a modal saying "no
    // internet" would pop up right on top of fully visible content.
    // Only the Google Maps tile layer itself needs a connection, and it
    // degrades gracefully (blank/grey tiles) rather than failing loudly.
    // The OfflineBanner above the map is the right signal for that.
    //
    // If you later add an action that genuinely requires connectivity
    // (e.g. "get directions", live traffic refresh), call
    // ConnectionDialogService.showNoInternetDialog(context) from that
    // action's handler instead of here.
  }
