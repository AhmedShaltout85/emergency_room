...
class _LabsReportsDashboardScreenState extends State<LabsReportsDashboardScreen>
  ...
  Widget build(BuildContext context) {
    ...
    // Nothing useful is on screen only when we're offline AND no lab has
    // been picked yet (so there are no charts to fall back on). In that
    // case show the full NoInternetWidget instead of the empty state.
    final showNoInternetWidget =
        !_isOnline && _isOnlineChecked && _selectedLabCode == null;
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _buildTabBar(context, isMobile),
          Expanded(
            child: showNoInternetWidget
                ? NoInternetWidget(onRetry: _checkConnectivity)
                : (_selectedLabCode == null
                    ? _buildEmptyState(context)
                    : TabBarView(
                        controller: _tabController,
                        children: List.generate(_tabItems.length, _buildChart),
